...
const CarritoLateral: React.FC<CarritoLateralProps> = ({
  ...
  const handleContinuarClick = () => {
    ...
    navigate("/pedido", { state: { cartItems: cartItems, usuarioId: usuarioId } });
  };
  ...
  const fetchCartItems = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://localhost:5000/api/carrito/carrito/${usuarioId}`);
      if (!response.ok) {
  ...
  useEffect(() => {
    if (show) { // Solo cargar si el carrito lateral está visible
      fetchCartItems();
    } else if (!show) {
  ...
  const handleRemoveItem = async (productoId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/carrito/carrito/eliminar?usuario_id=${usuarioId}&producto_id=${productoId}`, {
        method: 'DELETE',
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail ? JSON.stringify(errorData.detail) : errorData.error || `Error al eliminar el producto: ${response.statusText}`);
      }
  ...
  const handleEmptyCart = async () => {
    ...
    if (window.confirm("¿Estás seguro de que quieres vaciar todo el carrito?")) {
      try {
        const response = await fetch(`http://localhost:5000/api/carrito/carrito/vaciar/${usuarioId}`, {
          method: 'DELETE',
        });

        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Error al vaciar el carrito: ${response.statusText}`);
        }
        fetchCartItems(); // Recargar el carrito para actualizar la UI
      } catch (err: any) {
            ...
            <div className="cart-items-list mb-4">
              {cartItems.length === 0 ? (
                ...
                cartItems.map((item) => (
                  ...
                  >
                    <Image
                      src={`http://localhost:5000/static/productos/${item.imagen_url}`}
                      alt={item.nombre}
                      style={{
                        width: "80px",
                        height: "80px",
